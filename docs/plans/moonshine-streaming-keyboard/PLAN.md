...
| 4-redo | **Windowless Gboard-style dictation**: no bar, mic-key glow, live stable-chunk streaming, keyboard stays. Design: [WS4-REDO-DICTATION-DESIGN.md](WS4-REDO-DICTATION-DESIGN.md) · Step 1 plan: [WS4-REDO-STEP1-PLAN.md](WS4-REDO-STEP1-PLAN.md) | new `uix/voice/TextEditCoordinator.kt`, `HeadlessVoiceSession.kt`, `VoiceInputAction.kt`, `ActionBar.kt` | Step 1 SHIPPED (`1aa5e2a47`), on-device confirmed |
| 4-redo 1.5 | **Append-only dictation + UX hardening** (fixes on-device re-paste bug): per-burst frontier state machine ([state_machine.txt](../../Research/state_machine.txt)), VAD keep-listening, field-context caps, boundary spacing, animated glow ring, default voice system prompts. Plan: [WS4-REDO-STEP1.5-PLAN.md](WS4-REDO-STEP1.5-PLAN.md) | `TextEditCoordinator.kt`, `HeadlessVoiceSession.kt`, `ActionBar.kt`, voice settings defaults | in progress |
